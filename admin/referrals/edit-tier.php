    ...
    if (empty($name)) {
        ...
        try {
            $stmt = $pdo->prepare("
                ...
                SET name = ?, min_topup = ?, max_topup = ?, commission_percent = ?, free_shipping_vouchers = ?
                WHERE id = ?
            ");
            $stmt->execute([$name, $min_topup, $max_topup, $commission_percent, $free_shipping_vouchers, $tier_id]);

            $_SESSION['success'] = 'Commission tier updated successfully!';
            redirect('/admin/referrals/index.php');
...
$page_title = 'Edit Commission Tier';
...
<div class="page-header">
    ...
    <p>Update commission tier settings</p>
</div>
...
<div class="form-container">
    <form method="POST">
        <div class="form-group">
            ...
            <input type="text" id="name" name="name" required placeholder="e.g., Tier 1: Under 500K" value="<?php echo htmlspecialchars($tier['name']); ?>">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="number" id="min_topup" name="min_topup" min="0" step="1000" required placeholder="0" value="<?php echo $tier['min_topup']; ?>">
                <div class="help-text">Minimum first topup amount for this tier</div>
            </div>

            <div class="form-group">
                <label for="max_topup">Max Topup Amount (Rp)</label>
                <input type="number" id="max_topup" name="max_topup" min="0" step="1000" placeholder="Leave empty for unlimited" value="<?php echo $tier['max_topup'] ?? ''; ?>">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="number" id="commission_percent" name="commission_percent" min="0" max="100" step="0.01" required placeholder="5.00" value="<?php echo $tier['commission_percent']; ?>">
                <div class="help-text">Commission percentage (e.g., 5 for 5%)</div>
            </div>

            <div class="form-group">
                <label for="free_shipping_vouchers">Free Shipping Vouchers *</label>
                <input type="number" id="free_shipping_vouchers" name="free_shipping_vouchers" min="0" step="1" required placeholder="1" value="<?php echo $tier['free_shipping_vouchers']; ?>">
        ...
        <div class="button-group">
            ...
            <a href="/admin/referrals/delete-tier.php?id=<?php echo $tier_id; ?>" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this commission tier?')">Delete Tier</a>
        </div>
    </form>
</div>
